...
def sp_train_lines():
    # TODO consider moving to fire
    parser = argparse.ArgumentParser(
        description='build sentencepiece model on all the subset of the corpora')
    arg = parser.add_argument
    arg('corpora', nargs='+',
        help='corpus roots, containing train/valid/test splits')
    arg('sp_text', help='text file for sentencepiece model '
    ...
    if sp_text.exists():
        ...
        print(f'Reading corpora: {args.corpora}')
        for corpus_root in map(Path, args.corpora):
            corpus_paths = list(corpus_root.glob('**/*.txt'))
            if not corpus_paths:
                parser.error(f'Corpus {corpus_root} looks empty, '
                             f'no text files found')
            paths.extend(corpus_paths)
        try:
            with sp_text.open('wt', encoding='utf8') as sp_text_file:
...
def sp_encode_lines():
    ...
    arg = parser.add_argument
    arg('corpora', nargs='+',
        help='corpus root conatining text files')
    arg('sp_model', help='path to output model')
    arg('output', help='path to the output directory, '
                       'which will contain train.npy, valid.npy and test.npy')
    arg('train', type=int, help='train percentage', default=90)
    arg('valid', type=int, help='test percentage', default=10)
    arg('test', type=int, help='train percentage', default=0)
    args = parser.parse_args()

    total_pct = args.train + args.valid + args.test
    train_pct = args.train / total_pct
    valid_pct = args.valid / total_pct
    test_pct = args.test / total_pct
    ...
    for corpus_root in map(Path, args.corpora):
        ...
        def append_and_clear(x, split):
            encoded_splits[split].append(np.array(x, dtype=dtype))
            x.clear()

        for path in tqdm.tqdm(corpus_paths, desc=str(corpus_root)):
            encoded = []
            with path.open('rt', encoding='utf8') as f:
                for line in f:
                    encoded.extend(sp_model.EncodeAsIds(line))
                    ...
                    if rnd < train_pct:
                        split = 'train'
                    elif rnd < train_pct + valid_pct:
                        split = 'valid'
                    else:
                        split = 'test'

                    append_and_clear(encoded, split)
